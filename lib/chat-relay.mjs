...
let claudeBinaryOverride = null

/**
 * Set a user-configured path to the `claude` binary.
 * Clears the cached resolution so the next call re-evaluates.
 */
export function setClaudeBinaryOverride (path) {
  claudeBinaryOverride = path || null
...
async function resolveClaudeBinary () {
  ...
  // 0. User-configured override
  if (claudeBinaryOverride && existsSync(claudeBinaryOverride)) {
    resolvedClaudePath = claudeBinaryOverride
...
export function buildSystemPrompt (context) {
  ...
  if (context?.openFiles?.length) {
    const list = context.openFiles
      .map(f => `- ${f.path}${f.isDirty ? ' (unsaved changes)' : ''}`)
      .join('\n')
    parts.push(`\nOpen files:\n${list}`)
  }
 ...
 * @returns {ChatSession}
 */
export function createSession ({ rootPath, context, sessionId, message, model, effort, permissionMode }) {
  return new ChatSession({ rootPath, context, sessionId, message, model, effort, permissionMode })
}
...
export class ChatSession extends EventEmitter {
  constructor ({ rootPath, context, sessionId, message, model, effort, permissionMode }) {
    super()
